...
./cache/go/bin/stmgr ospkg create \
  -out 'out/ospkgs/stprov.zip' \
  -label='Provisioning Tool' \
  -kernel=out/artifacts/stboot.vmlinuz \
  -initramfs=out/artifacts/stprov.cpio.gz \
  -cmdline=''
...
# => out/stprov.iso
./cache/go/bin/stmgr uki create -format iso \
  -out out/stprov.iso \
  -kernel contrib/linuxboot.vmlinuz \
  -initramfs out/artifacts/stprov.cpio.gz \
  -cmdline=''
