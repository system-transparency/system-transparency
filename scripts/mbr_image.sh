...
default_name="stboot_mbr_installation.img"
output=
...
if [[ ! -d "${syslinux_dir}" ]];
then
  >&2 echo "\"${syslinux_dir}\" directory not found"
  exit 1
fi
...
dd bs=440 count=1 conv=notrunc if="${syslinux_dir}/bios/mbr/gptmbr.bin" of="${output}.tmp" status=none
