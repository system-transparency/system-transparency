...
stconfig create $WORKDIR/$MANIFEST -o $WORKDIR/$BOOTFILE || { echo -e "stconfig create $failed"; exit 1; }

echo "[INFO]: sign $BOOTFILE with example keys"
stconfig sign $WORKDIR/$BOOTFILE $WORKDIR/signing/signing-key-1.key $WORKDIR/signing/signing-key-1.cert || { echo -e "stconfig sign $failed"; exit 1; }
stconfig sign $WORKDIR/$BOOTFILE $WORKDIR/signing/signing-key-2.key $WORKDIR/signing/signing-key-2.cert || { echo -e "stconfig sign $failed"; exit 1; }
stconfig sign $WORKDIR/$BOOTFILE $WORKDIR/signing/signing-key-3.key $WORKDIR/signing/signing-key-3.cert || { echo -e "stconfig sign $failed"; exit 1; }
